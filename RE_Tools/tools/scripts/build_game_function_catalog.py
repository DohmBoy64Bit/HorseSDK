...
CURATED: list[dict] = [
    ...
    fn(
        ...
        hook={"safe_pre_call": False, "notes": "Per-frame; loader detour throttled 2s"},
    ),
    fn(
        "game_post_swap_hook",
        "Game_PostSwapHook",
        "0xBFFA0",
        "loop",
        "After SDL_GL_SwapWindow @ GameMain 0xBEB00; ImGui hook site",
        doc="RE_Tools/docs/GameLoop.md",
        status="verified",
        verification=["capstone"],
        parameters=[{"reg": "rcx", "type": "void *", "name": "ctx"}],
        hook={"safe_pre_call": False, "notes": "Render hook site; ImGui overlay future"},
    ),
    fn(
        "game_world_sim_step",
        "Game_WorldSimStep",
    ...
    fn(
        ...
        hook={"safe_pre_call": False, "notes": "Shop UI; loader detour throttled 500ms"},
    ),
    fn(
        "race_state_machine",
        "RaceStateMachine",
